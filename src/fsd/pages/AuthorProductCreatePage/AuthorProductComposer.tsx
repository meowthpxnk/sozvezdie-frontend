...
export const AuthorProductComposer = ({
    ...
    const hydratedInitialFormKeyRef = useRef<string | null>(null);
    ...
    useEffect(() => {
        ...
        const hydrateKey = JSON.stringify({
            ...
            subcategorySlug: initialForm.subcategorySlug,
            fandomSlug: initialForm.fandomSlug,
            imageIds: initialForm.images.map((image) => image.id),
        });

        if (hydratedInitialFormKeyRef.current === hydrateKey) {
            return;
        }

        hydratedInitialFormKeyRef.current = hydrateKey;
        setForm(initialForm);
    }, [initialForm]);
                        ...
                        <ModalActions>
                            <ModalConfirmButton
                                ...
                                onClick={() => {
                                    ...
                                    setSaving(true);
                                    const submitPromise = onSubmitForm(form);
                                    void submitPromise
                                        .then(() => {
                                            setConfirmModalOpen(false);
